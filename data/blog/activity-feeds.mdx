...
> Timestamps are implied here—an activity log isn't very valuable if it doesn't keep track of _when_ things happen.
...
This is... better, since it means that our routing can be managed in a template or something, where it belongs. But it sort of feels like only half a solution. What if our user is performing an action on some other model—like deploying a site? We can't just whack a `site_id` column into our database, though, since some feed items, like logins, won't pertain to sites.
